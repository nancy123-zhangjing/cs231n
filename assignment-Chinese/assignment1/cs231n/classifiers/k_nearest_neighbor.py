...
class KNearestNeighbor(object):
    ...
    def compute_distances_no_loops(self, X):
        ...
        #################################################################################
        #由a^2+b^2-2ab=(a-b)^2
        a_2 = (np.sum(X**2 , axis = 1 ,keepdims = True)) #这样就保持列向量了，而不是压成(500,)
        b_2 = (np.sum(self.X_train**2, axis = 1))#直接压成（5000，）
        ab = X@self.X_train.T
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            sorted_index = np.argsort(dists[i])#得到的是排序后的数据在未排序前的数据中的索引,即对应的是哪一个训练样本
            k_closest_index = sorted_index[:k]
            closest_y = self.y_train[k_closest_index]#y_train里面记录了5000个训练样本的类型，这里直接把数组当做索引也是ok
            #########################################################################
            # 代办:                                                                 #
            ...
            #########################################################################
            count = np.bincount(closest_y)#返回一个数组，count[i]记录的是标签i出现的次数
            y_pred[i] = np.argmax(count)#argmax返回的是最大值的索引
        return y_pred
