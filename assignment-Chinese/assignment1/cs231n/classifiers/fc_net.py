...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        b1 = self.params['b1']
        b2 = self.params['b2']
        x_hidden, cache1 = relu_affine_forward(X, w1, b1)
        ...
        #################################################################################
        #这是训练模式
        #为了方便后续solver的编码，把grads的键名和params设为一样的
        loss_no_l2, dx = softmax_loss(scores,y)
        #需要特别注意，这里正则化的时候，不能只加w2,w1也要加上
        loss = loss_no_l2 + 0.5 * self.reg * np.sum(w2 * w2)+0.5 * self.reg * np.sum(w1 * w1)
        dx1,dw2,db2 = affine_backward(dx,cache2)

        #一定要注意，梯度的计算和前向传播的顺序是反过来的
        grads['W2'] = dw2 + self.reg*w2
        grads['b2'] = db2
        _,dw1,db1 = relu_affine_backward(dx1,cache1)
