...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    _, H = h0.shape
    prev_h = h0
    #这里特别要注意，初始化时候的数据类型，默认的是float32,而x,Wx都是float64,这会导致我们把next_h强行降低精度变成float32,导致误差
    h = torch.zeros(N,T,H,dtype=x.dtype)
    for i in range(T ):
        x_in = x[ : , i , : ]
        next_h = rnn_step_forward(x_in, prev_h, Wx, Wh, b)
        h[ : , i , : ] = next_h
        prev_h = next_h
    ##############################################################################
    #                               你的代码结束                                 #
...
def word_embedding_forward(x, W):
    ...
    #花式索引就是这样的，假如说w的维度是[m,n,k],x的维度是[v,d],那w[x]的维度是[v,d,n,k]，相当于直接把被索引的数组看成字典了
    out = W[x]
