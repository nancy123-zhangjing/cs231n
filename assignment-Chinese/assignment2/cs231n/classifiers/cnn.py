...
class ThreeLayerConvNet(object):
    ...
    该网络处理形状为(N, C, H, W)的小批量数据，其中包含N个图像，每个图像的高度为H、宽度为W，且有C个输入通道。
    """
        x_1,cache1 = conv_relu_pool_forward(X,W1,b1,conv_param,pool_param)
        x_2,cache2 = affine_relu_forward(x_1,W2,b2)
        scores,cache3 = affine_forward(x_2,W3,b3)
        ############################################################################
        #                             你的代码结束                                 #
        ############################################################################
        ...
        loss = loss_no_l2 + 0.5 * self.reg *(np.sum(W1*W1) + np.sum(W2*W2) +np.sum(W3*W3))
        dx2,dW3,db3 = affine_backward(dout,cache3)
        dx1,dW2,db2 = affine_relu_backward(dx2,cache2)
        _,dW1,db1 = conv_relu_pool_backward(dx1,cache1)
