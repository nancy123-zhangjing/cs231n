...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(self.num_layers):
            w_name = 'W' + str(i + 1)
            b_name = 'b' + str(i + 1)
            self.params[w_name] = weight_scale * np.random.randn(all_dims[i],all_dims[i+1])
            self.params[b_name] = np.zeros(all_dims[i+1])
            if self.normalization is not None and i+1 < self.num_layers:
                gamma_name = 'gamma' + str(i+1)
                beta_name = 'beta' + str(i+1)
                
                self.params[gamma_name] = np.ones(all_dims[i+1])
                self.params[beta_name] = np.zeros(all_dims[i+1])                              #
        ############################################################################
    ...
    def loss(self, X, y=None):
        ...
        if self.normalization is None:
            ...
            for i in range(self.num_layers - 1):
                ...
                if self.use_dropout:
                    x_hidden,cachei = relu_dropout_affine_forward(x_hidden,Wi,bi,self.dropout_param)
                else:
                    x_hidden,cachei = relu_affine_forward(x_hidden,Wi,bi)
                caches.append(cachei)
            #最后一层不需要relu
            W = self.params['W' + str(self.num_layers)]
            b = self.params['b' + str(self.num_layers)]
            scores,cache = affine_forward(x_hidden,W,b)
            #注意最后一层的cache和前面的cache结构是不一样的
            caches.append(cache)
            ...
            for i in range(self.num_layers - 1):
                Wi = self.params['W' + str(i+1)]
                bi = self.params['b' + str(i+1)]
                bn_paramsi = self.bn_params[i]
                gammai = self.params['gamma' + str(i+1)]
                betai = self.params['beta' + str(i+1)]
                if self.use_dropout:
                    x_hidden,cachei = batch_relu_drop_affine_forward(x_hidden,Wi,bi,gammai,betai,bn_paramsi,self.dropout_param)
                else:
                    x_hidden,cachei = batchnorm_relu_affine_forward(x_hidden,Wi,bi,gammai,betai,bn_paramsi)
                caches.append(cachei)
            #最后一层不需要relu
            W = self.params['W' + str(self.num_layers)]
            b = self.params['b' + str(self.num_layers)]
            ...
            for i in range(self.num_layers - 1):
                ...
                betai = self.params['beta' + str(i+1)]
                if self.use_dropout:
                    x_hidden,cachei = layer_relu_drop_affine_forward(x_hidden,Wi,bi,gammai,betai,bn_paramsi,self.dropout_param)
                else:
                    x_hidden,cachei = layernorm_relu_affine_forward(x_hidden,Wi,bi,gammai,betai,bn_paramsi)
                caches.append(cachei)
            #最后一层不需要relu
            W = self.params['W' + str(self.num_layers)]
            b = self.params['b' + str(self.num_layers)]
        ...
        #反向传播部分
        if self.normalization is None:
            loss_no_l2, dout = softmax_loss(scores,y)
            loss = loss_no_l2
            for i in range(self.num_layers):
                W = self.params['W' + str(i + 1)]
                loss += 0.5 * self.reg * np.sum(W*W)
            dout,dw_last,db_last = affine_backward(dout,caches.pop())
            grads['W' + str(self.num_layers)] = dw_last + self.reg * self.params['W' + str(self.num_layers)]
            grads['b' + str(self.num_layers)] = db_last
            for i in range(self.num_layers - 2, -1,-1):
                if self.use_dropout:
                    dout,dwi,dbi = relu_dropout_affine_backward(dout,caches.pop())
                else:
                    dout,dwi,dbi = relu_affine_backward(dout,caches.pop())
                grads['W'+str(i+1)] = dwi + self.reg*self.params['W' + str(i+1)]
                grads['b'+str(i+1)] = dbi 
        elif self.normalization == "batchnorm":
                loss_no_l2, dout = softmax_loss(scores,y)
                loss = loss_no_l2
                ...
                for i in range(self.num_layers - 2, -1,-1):
                    if self.use_dropout:
                        dout,dwi,dbi,dgammai,dbetai = batch_relu_drop_affine_backward(dout,caches.pop())
                    else:
                        dout,dwi,dbi,dgammai,dbetai = batchnorm_relu_affine_backward(dout,caches.pop())
                    grads['W'+str(i+1)] = dwi + self.reg*self.params['W' + str(i+1)]
                    grads['b'+str(i+1)] = dbi 
                ...
                for i in range(self.num_layers - 2, -1,-1):
                    if self.use_dropout:
                        dout,dwi,dbi,dgammai,dbetai = layer_relu_drop_affine_backward(dout,caches.pop())
                    else:
                        dout,dwi,dbi,dgammai,dbetai = layernorm_relu_affine_backward(dout,caches.pop())
                    grads['W'+str(i+1)] = dwi + self.reg*self.params['W' + str(i+1)]
                    grads['b'+str(i+1)] = dbi 
                    grads['gamma'+str(i+1)] = dgammai
                    grads['beta'+str(i+1)] = dbetai
