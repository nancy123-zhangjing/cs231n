...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        #在这里，h是全过程所有的隐藏状态，形状为 (N, T, H)，
        # 而W_vocab(H,V),b_vocab(V,)的作用则是由隐藏状态映射到每个状态对应的词汇表得分
        #out(N,T,V)每个句子、在每个时间步、对词汇表中每个单词的全部预测得分
    ...
    def sample(self, features, max_length=30):
        ...
        #################################################################################################
        #首先根据提取出来的特征，经过变换得到初始隐藏状态
        prev_h = features @ W_proj + b_proj
        #然后要给每一个图片生成一个start作为初始输入
        #在深度学习中，所有表示Index的张量，数据类型必须是整数，而在 PyTorch 里对应的就是 torch.long
        current_word = torch.full((N,), self._start, dtype=torch.long, device=features.device)
        #然后就可以开始循环
        for t in range(max_length):
            #W_embed里面储存的是所有单词对应的词向量
            ...
            #找到得分最高的那个位置的索引
            #scores矩阵的维度是(N,V),现在压成(N,)
            next_word_index = torch.argmax(scores,dim=1)
            captions[:,t] = next_word_index
            #注意看一开始初始化时候的current_word对应的也是start的索引
