...
def get_similarity_no_loop(text_features, image_features):
    ...
    ############################################################################
    text_norm = torch.linalg.norm(text_features,dim=1,keepdim=True)
    image_norm = torch.linalg.norm(image_features,dim=1,keepdim=True)
    norm_matrix = torch.matmul(text_norm,image_norm.transpose(1,0))
    dot_matrix = torch.matmul(text_features,image_features.transpose(1,0))
    similarity = dot_matrix / norm_matrix
    ############################################################################
    #                             代码结束部分                                  #
    ############################################################################
...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    similarity_matrix = get_similarity_no_loop(text_features,image_features)
    index = torch.argmax(similarity_matrix,dim=0)
    index = index.tolist()
    pred_classes = [class_texts[i] for i in index]
    ############################################################################
    #                             代码结束部分                                  #
    ############################################################################
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        self.device = device
        #然后要对图像进行预处理
        processed_images = [clip_preprocess(Image.fromarray(img)).unsqueeze(0)
            for img in images
        ]
        images_tensor = torch.cat(processed_images, dim=0).to(device)
        image_features = clip_model.encode_image(images_tensor)
        #由于题目说，不用之前的相似度函数，因为不想对每个文本重新计算图像特征，相当于要在init的时候就计算好，那么应该在这里先进行一下归一化
        image_norm = torch.linalg.norm(image_features,dim=1,keepdim=True)
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        similarity_matrix = torch.matmul(text_features_norm,image_features_norm.transpose(1,0)) #(N,M)
        _, top_indices = torch.topk(similarity_matrix, k=k, dim=1, largest=True) #现在还是一个二维[[idx1, idx2]]
...
class DINOSegmentation:
    ...
    def train(self, X_train, Y_train, num_iters=500):
        ...
        for i in range(num_iters):
            ...
            loss = self.loss(scores,Y_train)
            self.optimizer.zero_grad()
            loss.backward()
            self.optimizer.step()
            if (i + 1) % 100 == 0:
                print(f"迭代 [{i+1}/{num_iters}]次, Loss: {loss.item():.4f}")
        ############################################################################
        #                             代码结束部分                                  #
        ############################################################################
        pass
