...
class Unet(nn.Module):
    def __init__(
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out_ups):
            ...
            ##################################################################
            #由于在上采样的时候需要和之前同一级别的下采样的图片进行拼接，而且是依通道拼接，所以输入的通道数是两倍
    ...
    def cfg_forward(self, x, time, model_kwargs={}):
        ...
        uncond_pred = self.forward(x,time,model_kwargs_uncond)
    ...
    def forward(self, x, time, model_kwargs={}):
        ...
        ##################################################################
        save = []
        for res1,res2,downs in self.downs:
            x = res1(x,context)
            save.append(x)
            x = res2(x,context)
            save.append(x)
        ...
        for ups,res1,res2 in self.ups:
            x = ups(x)
            x = torch.cat([x,save.pop()],dim=1) #是从通道的维度拼接
            x = res1(x,context)
            x = torch.cat([x,save.pop()],dim=1)
            x = res2(x,context)
