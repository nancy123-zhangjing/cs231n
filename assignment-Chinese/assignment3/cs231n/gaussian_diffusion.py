...
class GaussianDiffusion(nn.Module):
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        ...
        coef1 = extract(self.sqrt_alphas_cumprod,t,x_t.shape)
        coef2 = extract(self.sqrt_one_minus_alphas_cumprod,t,x_t.shape)
        x_start =(x_t - coef2 * noise) / coef1
        ####################################################################
        return x_start
    ...
    def predict_noise_from_start(self, x_t, t, x_start):
        ...
        coef1 = extract(self.sqrt_alphas_cumprod,t,x_t.shape)
        coef2 = extract(self.sqrt_one_minus_alphas_cumprod,t,x_t.shape)
        pred_noise = (x_t - coef1 * x_start) / coef2
        ####################################################################
        return pred_noise
    ...
    def p_sample(self, x_t, t: int, model_kwargs={}):
        ...
        #      并采样x_{t-1}。
        ##################################################################
        x_predict = self.model(x_t,t,model_kwargs)
        #要搞明白，unet实际上有两个功能，这取决于在计算loss的时候，到底是以noise为目标还是以x0为目标，
        # 如果以x0为目标，那么吐出来的就是想要努力接近x0，否则就是走向noise的
        #并且这个方向只是一个理想化的大方向，我们并不能一步到位走下去
        if self.objective == "pred_noise":
            x_start = self.predict_start_from_noise(x_t,t,x_predict)
            x_noise = x_predict
        else:
            x_start = x_predict
            x_noise = self.predict_noise_from_start(x_t,t,x_start)
        x_start = torch.clamp(x_start, min=-1., max=1.)
        #计算出下一步落脚点的均值和标准差
        posterior_mean, posterior_std = self.q_posterior(x_start, x_t, t)
        #然后我们下一步的方向仍然是根据这个均值和标准差随机采样得到的
        #所以我们还需要生成一个noise,来随机化下一步的方向
        noise = torch.randn_like(x_t)
        #但是最后一步是不需要加噪声的
        nonzero_mask = (t != 0).float().view(-1, 1, 1, 1)
        x_tm1 = posterior_mean + posterior_std * noise * nonzero_mask
    ...
    def q_sample(self, x_start, t, noise):
        ...
        # 大约3行代码。
        ####################################################################
        #sqrt_alphas_cumprod和sqrt_one_minus_alphas_cumprod是(1000,)的数组，里面放了每一个时间步对应的参数
        #extract函数的作用是提取出对应时间t的参数并调整好维度
        #公式为x_t = sqrt(alpha_bar_t) * x_0 + sqrt(1 - alpha_bar_t) * noise
    ...
    def p_losses(self, x_start, model_kwargs={}):
        ...
        x_predict = self.model(x_t,t,model_kwargs)
        #然后要计算加权MSE,应该是先把mse算出来，但是先保留维度，然后再乘以weight
        loss = torch.mean((x_predict - target)**2 * loss_weight)
        ####################################################################
