...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        # 参考：我们的解决方案少于5行代码。
        ############################################################################
        # 这里还是要用到pytorch的广播机制，这样可以简化运算
        #因为我们的赋值是有规律的，i就是一串连续的数字，而j则是分为了奇数和偶数
        #直接生成的是一列，从0一直到max_len
        position = torch.arange(max_len).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, embed_dim, 2) * (-math.log(10000.0) / embed_dim))
        #观察公式可得，其实奇数和偶数可以合并为一个表达式
    ...
    def forward(self, x):
        ...
        # 这只需要几行代码。
        ############################################################################
        #因为pe的维度是[1,max_len,embed_dim],实际上我们只要取前s个
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        """
        N, S, E = query.shape
        N, T, E = key.shape    # 显式从 key 拿到它的序列长度 T
        # 创建一个占位符，将在下面的代码中覆盖
        ...
        #因为只有这里转置一下，后面才好相乘
        #因为torch,matmul的规则就是后面两个维度相乘，前面的维度，如果是一样的就不变，如果其中有一个是1，就变成不是1的那个
        query_heads = Query.view(N,S,H,D).transpose(1,2) # (N,H,S,D)
        key_heads = Key.view(N,T,H,D).transpose(1,2) # (N,H,T,D)
        value_heads = Value.view(N,T,H,D).transpose(1,2) # (N,H,T,D)
        scale = math.sqrt(D)
        weight = torch.matmul(query_heads,key_heads.transpose(2,3)) /scale#(N,H,S,T)
        #至于mask,我们需要抹去mask为0的位置的影响，但是注意，不可以先过softmax再过mask，因为softmax相当于是一个分配注意力权重的过程
        # 如果先过softmax，然后再用mask把不要的地方置零，那么其实注意力已经被抢走一部分了，所以我们必须在过softmax之前就把不要的位置处理掉
        #这样相当于是把原来是0的变成-inf,把原来是1的变成0
        if attn_mask is not None:
            weight = weight.masked_fill(attn_mask == 0, float('-inf'))
        final_weight = self.attn_drop(F.softmax(weight ,dim = -1))
        y = torch.matmul(final_weight,value_heads) # (N,H,S,D)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory, tgt_mask=None):
        ...
        - memory：编码器最后一层的输出序列，形状为(N, S, D)，代表从句子中提取出来的整体特征
        ...
        # 自注意力块（参考实现）
        shortcut = tgt
        tgt = self.self_attn(query=tgt, key=tgt, value=tgt, attn_mask=tgt_mask) #这里可以理解为实际上是在调用multiheadattention的forward函数
        ...
        ############################################################################
        shortcut1 = tgt
        #按照transformer的结构图，应该是编码器输出(memory)作为key和value,解码器输出(query)作为query
        tgt = self.cross_attn(query=tgt,key=memory,value=memory)
        tgt = self.dropout_cross(tgt)
        tgt = tgt + shortcut1 #残差连接
        tgt = self.norm_cross(tgt)

        shortcut2 = tgt
        tgt = self.ffn(x=tgt)
        tgt = self.dropout_ffn(tgt)
        tgt = tgt + shortcut2 #残差连接
        tgt = self.norm_fnn(tgt)
        ############################################################################
        #                             代码结束部分                                  #
        ############################################################################
...
class PatchEmbedding(nn.Module):
    ...
    def __init__(self, img_size, patch_size, in_channels=3, embed_dim=128):
        ...
        self.num_patches = (img_size // patch_size) ** 2 #类似于前面的序列长度T
        self.patch_dim = patch_size * patch_size * in_channels #类似于前面的每个词的ID

        # 补丁展平后的线性投影（到嵌入维度）
        self.proj = nn.Linear(self.patch_dim, embed_dim) #把每个小块转换成特征向量
