...
def load_pickle(f):
    """加载pickle文件，兼容Python 2和Python 3"""
    version = platform.python_version_tuple()  # 获取Python版本
    if version[0] == "2":
        return pickle.load(f)  # Python 2直接加载
    elif version[0] == "3":
        return pickle.load(f, encoding="latin1")  # Python 3指定编码
...
def load_CIFAR_batch(filename):
    ...
    with open(filename, "rb") as f:
        ...
        # 重塑并转换维度：(10000, 3, 32, 32) -> (10000, 32, 32, 3)，并转为float类型
        X = X.reshape(10000, 3, 32, 32).transpose(0, 2, 3, 1).astype("float")
        Y = np.array(Y)  # 转为NumPy数组
        return X, Y
...
def load_tiny_imagenet(path, dtype=np.float32, subtract_mean=True):
    ...
    返回：一个包含以下条目的字典：
    - class_names: 列表，其中class_names[i]是字符串列表，给出加载数据集中
      第i类的WordNet名称。
    ...
    # 使用words.txt获取每个类别的名称
    with open(os.path.join(path, "words.txt"), "r") as f:
        wnid_to_words = dict(line.split("\t") for line in f)  # 解析wnid与名称的映射
        for wnid, words in wnid_to_words.items():
            wnid_to_words[wnid] = [w.strip() for w in words.split(",")]  # 拆分多个名称
    class_names = [wnid_to_words[wnid] for wnid in wnids]  # 按标签顺序存储类别名称
    ...
    for i, wnid in enumerate(wnids):
        if (i + 1) % 20 == 0:
            print("正在加载第 %d / %d 个同义词集的训练数据" % (i + 1, len(wnids)))
        # 为了确定文件名，需要打开boxes文件
        ...
        for j, img_file in enumerate(filenames):
            ...
            if img.ndim == 2:
                ## 灰度图像，添加通道维度
                img.shape = (64, 64, 1)
            X_train_block[j] = img.transpose(2, 0, 1)  # 转换为通道在前的格式
    ...
    with open(os.path.join(path, "val", "val_annotations.txt"), "r") as f:
        ...
        for i, img_file in enumerate(img_files):
            img_file = os.path.join(path, "val", "images", img_file)  # 图像路径
            img = imread(img_file)  # 读取图像
            if img.ndim == 2:
                img.shape = (64, 64, 1)  # 灰度图像添加通道维度
            X_val[i] = img.transpose(2, 0, 1)  # 转换为通道在前的格式

    # 接下来加载测试图像
    # 学生版本可能没有测试标签，因此需要遍历图像目录中的文件
    img_files = os.listdir(os.path.join(path, "test", "images"))  # 获取所有测试图像文件名
    X_test = np.zeros((len(img_files), 3, 64, 64), dtype=dtype)
    for i, img_file in enumerate(img_files):
        img_file = os.path.join(path, "test", "images", img_file)  # 图像路径
        img = imread(img_file)  # 读取图像
        if img.ndim == 2:
            img.shape = (64, 64, 1)  # 灰度图像添加通道维度
        X_test[i] = img.transpose(2, 0, 1)  # 转换为通道在前的格式

    y_test = None
    y_test_file = os.path.join(path, "test", "test_annotations.txt")
    if os.path.isfile(y_test_file):  # 如果测试标签文件存在
        with open(y_test_file, "r") as f:
            img_file_to_wnid = {}
            for line in f:
                line = line.split("\t")
                img_file_to_wnid[line[0]] = line[1]  # 建立文件名到wnid的映射
...
def load_models(models_dir):
    """
    从磁盘加载保存的模型。将尝试对目录中的所有文件进行反序列化；
    任何反序列化出错的文件（如README.txt）将被跳过。

    输入：
    - models_dir: 包含模型文件的目录路径字符串。
      每个模型文件是一个带有'model'字段的pickle字典。
...
def load_imagenet_val(num=None):
    ...
    np.load = np_load_old  # 恢复默认设置
    X = f["X"]  # 图像数据
    y = f["y"]  # 标签数据
    class_names = f["label_map"].item()  # 类别名称映射
    if num is not None:
        X = X[:num]
        y = y[:num]
