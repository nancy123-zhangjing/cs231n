...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        ############################################################################
        #本来captions只是每个句子中每个词对应的索引
        tgt_emb = self.embedding(captions) #(N,T,W)
        #通过词嵌入层，得到了每个句子每个词的特征向量
        #这里也是一样的，会直接调用PositionalEncoding这个类中的forward函数
        #至于为什么forward函数可以直接调用，这实际上和继承的module类有关，在这个类中，把forward函数加了__call__，
        # 这就导致forward必须直接用一个括号来调用，而如果类里面还有其他的函数，则需要显式调用
        tgt_pos = self.positional_encoding(tgt_emb) #(N,T,W)
        feature_word_dim = self.visual_projection(features) #(N,W)
        #强行升维,因为前面我们都在默认是在处理句子，这里我们需要处理图片，我们的编码器得到的是图片的信息，
        # 而我们图片的信息不是像句子那样，分成T步，一张图片就是一个特征向量，但是为了transformer可以正常调用，我们需要手动升维
        memory = feature_word_dim.unsqueeze(1)#(N,1,W)
        #掩码矩阵应该是一个(T,T)的下三角矩阵
        tgt_mask = torch.tril(torch.ones((T, T)))
        decoder_out = self.transformer(tgt = tgt_pos,memory = memory,tgt_mask = tgt_mask)#(N,T,W)
        scores = self.output(decoder_out) # (N,T,V),这里输出的是原始得分
        ############################################################################
        #                             代码结束部分                                  #
        ############################################################################
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # 5. 将其通过线性层生成类别logits。                                         #
        ############################################################################
        x_emb = self.patch_embed(x) #(N,T,W)
        T = x_emb.shape[1]
        x_pos = self.positional_encoding(x_emb)
        out = self.transformer(src = x_pos) #(N,T,W)
        mean_out = torch.mean(out,dim = 1)
        logits = self.head(mean_out)
        ############################################################################
        #                             代码结束部分                                  #
        ############################################################################
